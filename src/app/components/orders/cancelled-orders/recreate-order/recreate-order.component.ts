...
@Component({
  selector: 'app-recreate-order',
  templateUrl: './recreate-order.component.html',
  styleUrls: ['./recreate-order.component.css']
})
export class RecreateOrderComponent implements OnInit {
  ...
  constructor(
    ...
    public cancelledOrderService: CancelledOrdersService,
    public notificationService: NotificationService,
    public dialogRef: MatDialogRef<RecreateOrderComponent>,
  ...
  ngOnInit() {
    this.cancelledOrderService.getProducts().subscribe((products:any)=>{
    ...
    this.cancelledOrderService.getColorVariants().subscribe((response)=>{
      this.colorVariants=response;
    }) ;
    this.cancelledOrderService.getSizes().subscribe((response)=>{
      this.sizeVariants=response;
    });
    this.cancelledOrderService.getProvinces().subscribe((response)=>{
      this.provinces=response;
    });
    this.cancelledOrderService.getCustomers().subscribe((response :any)=>{
    ...
    //initialize form inputs 
      this.cancelledOrderService.getAffiliateSellerOrderById(this.id).subscribe((outereRsponse:any)=>{
        ...
        this.customerInput.nativeElement.value = "";
          ...
          this.cancelledOrderService.getOneProductFromStore({
  ...
  onSubmit() {
    let dataToSend = {_id: this.id}
    this.cancelledOrderService.changeOrderFromCancelledToCreated(dataToSend).subscribe((status)=>{
      if(status==true)
          this.notificationService.success(":: Order Recreated Successfully");
          else
