...
export class AssignedOrdersService {
  ...
  getOnlyAssignedOrders() {
    return this.http.get(`${systemSettings.serverURL}/orders/getOnlyAssignedOrders`)
  }
  ...
  reAssignOrderTo(dataToSend) {
    return this.http
      .post(`${systemSettings.serverURL}/orders/onlyChangeOrderEmployee`, dataToSend)
      .pipe(
        map((response: any) => {
          if (response.message == true) return true;
          return false;
